...
def get_front_url(url):#获取以及页面的url
    ...
    url_link=soup.xpath("//div[@class='ns-bg-wrap']/div/a/@href")
    url_name=soup.xpath("//div[@class='ns-bg-wrap']/div/a/text()")
    url_link=url_link[1:]
    url_name=url_name[1:]
...
def get_the_content(url,name):#得到一个URL列表，获取列表中每个链接对应的页面的文字信息
    with open(r"c.csv","a+",encoding="utf-8",errors="ignore",newline="") as csvfile:
        ...
        for u in url:
            ...
            try:
                ...
                continue
            soup = etree.HTML(text)
            url_content= soup.xpath("//div[@class='post_text']/p/text()")
...
def set_browser():
    ...
    driver = webdriver.PhantomJS(executable_path=r"/home/lhn/pythonfile/xpath_spider/phantomjs-2.1.1-linux-x86_64/bin/phantomjs",
...
def get_total_url_list(driver,url_link):#返回二级页面中所有的标签列表
    ...
    for id, item in enumerate(url_link):
        ...
        url = soup.xpath("//div[@class='ndi_main']/div[@class='data_row news_article clearfix ']/a/@href")#返回一个url列表

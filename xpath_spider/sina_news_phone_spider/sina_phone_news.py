...
def get_front_url(url):#获取以及页面的url
    ...
    url_link=soup.xpath("//nav[@class='hd_nav']/a[@class='hd_nav_a']/@href")
    url_name=soup.xpath("//nav[@class='hd_nav']/a[@class='hd_nav_a']/text()")
    url_link=url_link[1:]
    url_name=url_name[1:]
...
def get_the_content(url,name):#得到一个URL列表，获取列表中每个链接对应的页面的文字信息
    with open(r"b.csv","a+",encoding="utf-8",errors="ignore",newline="") as csvfile:
        ...
        for u in url:
            ...
            try:
                ...
                continue
            soup = etree.HTML(text)
            url_content= soup.xpath("//article[@class='art_box']/p/text()")
...
def get_more_links(driver,num):#num为点击次数
    for i in range(num):
        try:
            ac = driver.find_element_by_xpath("//section[@class='s_card j_tianyi']/a[last()]")
...
def get_total_url_list(driver,url_link):#返回二级页面中所有的标签列表
    ...
    for id, item in enumerate(url_link):
        ...
        # url = soup.xpath("//section[@class='s_card j_tianyi']/a[@class='m_f_a']/@href")#返回一个url列表
        url = soup.xpath("//a[@class='m_f_a']/@href")#返回一个url列表
